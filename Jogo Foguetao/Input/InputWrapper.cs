...
namespace Input
{
    internal struct AllInputButtons
    {
        ...
        {
            if (Keyboard.GetState().IsKeyDown(key)) return ButtonState.Pressed;
            if (GamePad.GetState(PlayerIndex.One).IsConnected) return gamePadButtonState;
    ...
    internal struct AllInputTriggers
    {
        ...
        {
            if (Keyboard.GetState().IsKeyDown(key)) return kKeyTriggerValue;
            if (GamePad.GetState(PlayerIndex.One).IsConnected) return gamePadTrigger;
            return 0f;
        }
    ...
    internal struct AllThumbSticks
    {
        ...
        private Vector2 ThumbStickState(Vector2 thumbStickValue, Keys up, Keys down, Keys left, Keys right)
        {
            ...
            if (Keyboard.GetState().IsKeyDown(up)) r.Y -= kKeyDownValue;
            if (Keyboard.GetState().IsKeyDown(down)) r.Y += kKeyDownValue;
            if (Keyboard.GetState().IsKeyDown(left)) r.X += kKeyDownValue;
            if (Keyboard.GetState().IsKeyDown(right)) r.X -= kKeyDownValue;
            return r;
        }
        public Vector2 Left
        { get { return ThumbStickState(GamePad.GetState(PlayerIndex.One).ThumbSticks.Left, kLeftThumbStickUp, kLeftThumbStickDown, kLeftThumbStickLeft, kLeftThumbStickRight); } }
        public Vector2 Right
        { get { return ThumbStickState(GamePad.GetState(PlayerIndex.One).ThumbSticks.Right, kRightThumbStickUp, kRightThumbStickDown, kRightThumbStickLeft, kRightThumbStickRight); } }
    }
